...
class Settings(BaseSettings):
    ...
    base_backoff_seconds: float = Field(
        default=0.25, validation_alias="WORKER_BASE_BACKOFF_SECONDS"
    )
    ...
    db_write_timeout_seconds: float = Field(
        default=2.0, validation_alias="WORKER_DB_WRITE_TIMEOUT_SECONDS"
    )

    db_connect_max_retries: int = Field(
        default=30, validation_alias="WORKER_DB_CONNECT_MAX_RETRIES"
    )
    db_connect_base_backoff_seconds: float = Field(
        default=0.5, validation_alias="WORKER_DB_CONNECT_BASE_BACKOFF_SECONDS"
    )
    db_connect_max_backoff_seconds: float = Field(
        default=5.0, validation_alias="WORKER_DB_CONNECT_MAX_BACKOFF_SECONDS"
    )
